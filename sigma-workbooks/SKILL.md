...
description: >-
  Build, edit, and iterate on Sigma workbook specs — the YAML/JSON definition
  of pages, layout, controls, charts, KPIs, tables, formulas, and sources — by
  calling the Sigma REST API directly. Use when the user wants to construct a
  dashboard or workbook from a spec, add or modify pages/elements/controls/
  formulas, validate a spec before submission, or iterate on pre-release
  workbook spec endpoints. Requires an SIGMA_API_TOKEN — obtain via the
  sigma-api skill first.
---
