...
- `waterfall-chart` — same `source`/`columns`/`xAxis`/`yAxis` skeleton as `bar-chart`, plus running-total-specific fields: `startPoint` (the opening bar — `value.type: first-value | constant | column`, plus `label`/`visibility`), `splitBy: { id }` (optional column that splits the waterfall into side-by-side series), `grouping: stacked | clustered` (default `stacked`), `waterfallColors: { increase, decrease, total }` (per-bar-type color, hex or theme ref), `waterfallShape: { calculation: sum | difference, connectorLine, connectorLineColor, subtotal, endTotal }` (bar-combination mode, connector lines, subtotal/end-total label overrides). **Gotcha (live-verified 2026-08-03):** `waterfallShape.subtotal`/`endTotal` reject with a 400 — `"only supported when the chart has split-by or multiple Y-axis series"` — unless the chart has `splitBy` or 2+ `yAxis.columnIds`. `waterfallColors` and `splitBy`+`grouping: clustered` confirmed rendering as expected (colored bars per increase/decrease/total, side-by-side series per category) on a real create+readback+screenshot.
