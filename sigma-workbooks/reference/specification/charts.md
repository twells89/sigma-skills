...
- `waterfall-chart` — same `source`/`columns`/`xAxis`/`yAxis` skeleton as `bar-chart`, plus running-total fields (`startPoint`, `splitBy`, `grouping`, `waterfallColors`, `waterfallShape`). **Gotcha (live-verified 2026-08-03):** `waterfallShape.subtotal`/`endTotal` reject with a 400 unless the chart has `splitBy` or 2+ `yAxis.columnIds`.
