...
description: >-
  ...
  conversions are handled by the matching `*-to-sigma` skill in
  sigma-migration-skills (Alteryx `.yxmd` → `alteryx-to-sigma`, local
  converter, never a hosted MCP) or, for dbt / LookML / Tableau / Power BI /
  Snowflake semantic views, the Sigma data-model converter MCP / browser
  tool. Point users there when they paste source-format input.
...
| File | When to load |
|------|--------------|
| `reference/workflows/discover.md` | **Read first when starting a new model.** Find the connection, resolve table paths, list columns, declare the expected grain, and prove source/join cardinality before drafting. Without this, a spec can compile while every aggregate is wrong. |
